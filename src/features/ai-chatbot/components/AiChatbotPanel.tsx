...
export function AiChatbotPanel({ suggestedPrompts, isGuest, initialConversations }: AiChatbotPanelProps) {
  ...
  const selectConversation = async (id: string) => {
    setActiveId(id);
    setError(null);
    const result = await loadChatMessagesAction(id);
    if (result.ok) {
  ...
  const sendMessage = async (text: string, retry = false) => {
    ...
    try {
      ...
      const json = (await res.json()) as ChatApiSuccess | ChatApiFailure;
      ...
      const loaded = await loadChatMessagesAction(convId);
      if (loaded.ok) {
        setMessages(loaded.data);
      } else {
        setMessages((prev) => {
          const withoutLocal = prev.filter((m) => !m.id.startsWith("local-u-"));
    ...
    <Stack gap="md" className={styles.page}>
      ...
      <div className={styles.layout}>
        <Paper className={styles.sidebar} p="sm" radius="md">
          ...
          <ScrollArea className={styles.sidebarScroll} type="auto">
            <Stack gap={4}>
